...
class MaximalCodingRateReduction(torch.nn.Module):
    ...
    def compute_discrimn_loss_empirical(self, W):
        ...
        p, m = W.shape
        I = torch.eye(p, dtype=W.dtype).to(W.device)
        scalar = p / (m * self.eps)
        logdet = self.logdet(I + self.gam1 * scalar * W.matmul(W.conj().T))
        return logdet / 2.
    ...
    def compute_compress_loss_empirical(self, W, Pi):
        ...
        for j in range(k):
            ...
            log_det = self.logdet(I + scalar * W.matmul(Pi[j].to(W.dtype)).matmul(W.conj().T))
    ...
    def forward(self, X, Y, num_classes=None):
        ...
        total_loss_empi = torch.real(total_loss_empi)

        return (total_loss_empi,
                [discrimn_loss_empi.real.item(),
                compress_loss_empi.real.item()])
