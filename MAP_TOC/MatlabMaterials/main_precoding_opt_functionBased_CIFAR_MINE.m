...
% B = 10*1e3;  % 10 kHz
K = 2;  % num. of devices
N_t_k = 3;  % num. of transmit antennas
N_t = N_t_k * K;
N_r = 6;  % num. of receive antennas
T = 2;  % num. of time slots
...
Var_Vec = [1    10    15    20        30 40];[5    10    15    20    25    30];
eps = 1e-3;
ITERATIONS = 1000;
...
for iter = 1:ITERATIONS
    for varInx = 1:length(Var_Vec)
        ...
        [V, mcr2_all] = Training_MCR2_precoder( ...
            H, C, CLS_cov, p, delta_0, P_k, K, N_t_k,N_r, T, D,P,L,H_k_all);
        %%%%%%MAP receiver %%%%%%%
        ...
        disp(['Acc MCR2  = ' num2str(Acc_MCR2_mean(1,varInx))])
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%% LMMSE Precoder%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        [V, mse_all] = LMMSE_precoder(H, C, delta_0, P_k, K, N_t_k,N_r, T, D,P,H_k_all);
        %%%%%%MAP receiver %%%%
    ...
    close all;plot(Var_Vec,Acc_Ours_mean,'LineWidth',2)
    hold all;plot(Var_Vec,Acc_MCR2_mean,'LineWidth',2)
    hold all;plot(Var_Vec,Acc_LMMSE_mean,'LineWidth',2)
    hold all;plot(Var_Vec,Acc_Random_mean,'LineWidth',2)
...
function [V,mse_all] = LMMSE_precoder(H, C,  delta_0, P_k, K, N_t_k,N_r, T, D,P,H_k_all)
